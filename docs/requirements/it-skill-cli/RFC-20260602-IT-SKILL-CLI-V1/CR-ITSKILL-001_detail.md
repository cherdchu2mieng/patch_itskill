...
- [ ] New repository `itinfosv/it-skill-cli` initialized successfully.
- [ ] `package.json` contains correct `itinfosv` metadata and `it-skill` binary name.
- [ ] `README.md` reflects the new identity.
- [ ] All instances of the old branding are replaced in the source code.
- [ ] Project builds successfully via `bun run build`.
