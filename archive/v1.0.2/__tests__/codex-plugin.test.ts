...
const TEST_HOME = join(tmpdir(), `it-codex-plugin-${Date.now()}`);
...
const MOCK_SKILLS: Skill[] = [
  {
    ...
    path: join(TEST_HOME, "skills", "close-rfc"), // non-existent path — prompt.md copy is skipped gracefully
  },
  {
    name: "build-patch",
    description: "Session orientation and awareness",
    path: join(TEST_HOME, "skills", "build-patch"),
  },
  {
...
describe("getCodexMarketplaceDir", () => {
  it("returns path under the given homeDir", () => {
    ...
    expect(dir).toBe(
      join(TEST_HOME, ".codex", ".tmp", "bundled-marketplaces", "it-skill")
    );
  });
});
...
describe("installCodexPluginMarketplace", () => {
  ...
  it("creates manifest.toml with correct name and version", async () => {
    ...
    expect(content).toContain(`name = "it-skill"`);
  ...
  it("plugin.toml includes the skill description", async () => {
    const pluginTomlPath = join(MARKETPLACE_DIR, "plugins", "close-rfc", "plugin.toml");
    const content = await readFile(pluginTomlPath, "utf-8");
    expect(content).toContain("Session retrospective");
  });

  it("adds [marketplaces.it-skill] to config.toml", async () => {
    const content = await readFile(CONFIG_PATH, "utf-8");
    expect(content).toContain(`[marketplaces.it-skill]`);
  ...
  it("enables non-hidden skills as [plugins.*@it-skill]", async () => {
    const content = await readFile(CONFIG_PATH, "utf-8");
    expect(content).toContain(`[plugins."close-rfc@it-skill"]`);
    expect(content).toContain(`[plugins."build-patch@it-skill"]`);
  ...
  it("does NOT register hidden skills as plugins in config.toml", async () => {
    const content = await readFile(CONFIG_PATH, "utf-8");
    expect(content).not.toContain(`[plugins."secret-internal@it-skill"]`);
  });
  ...
  it("is idempotent — re-running does not duplicate config.toml sections", async () => {
    ...
    const marketplaceCount = (content.match(/\[marketplaces\.it-skill\]/g) || []).length;
    expect(marketplaceCount).toBe(1);

    // Count occurrences of the rrr plugin section header
    const rrrPluginCount = (content.match(/\[plugins\."close-rfc@it-skill"\]/g) || []).length;
...
describe("installCodexPluginMarketplace (real skills)", () => {
  ...
  it("registers real skills in config.toml", async () => {
    ...
    expect(config).toContain(`[marketplaces.it-skill]`);

    for (const skill of tested) {
      if (skill.hidden) continue;
      expect(config).toContain(`[plugins."${skill.name}@it-skill"]`);
    }
  });
});
...
describe("installCodexPluginMarketplace — JSON format (Codex 0.130+)", () => {
  const JSON_TEST_HOME = join(tmpdir(), `it-codex-plugin-json-${Date.now()}`);
  ...
  it("creates .agents/plugins/marketplace.json (required by Codex 0.130 marketplace add)", async () => {
    ...
    expect(parsed.name).toBe("it-skill");
    ...
    expect(names).toContain("close-rfc");
    expect(names).toContain("build-patch");
  ...
  it("plugin.json contains required fields (name, version, description, skills, interface)", async () => {
    ...
    expect(parsed.name).toBe("close-rfc");
    ...
    expect(parsed.interface.displayName).toBe("close-rfc");
  ...
  it("plugin.json shortDescription truncates long descriptions", async () => {
    ...
    const market = join(dir, ".codex", ".tmp", "bundled-marketplaces", "it-skill");
    await writeFile(cfg, "");
  ...
  it("registers marketplace + plugins in config.toml (same as TOML mode)", async () => {
    ...
    expect(config).toContain(`[plugins."close-rfc@it-skill"]`);
    expect(config).toContain(`[plugins."build-patch@it-skill"]`);
    // Hidden skill must not be registered
    expect(config).not.toContain(`[plugins."secret-internal@it-skill"]`);
  });
  ...
  it("cleans up stale TOML files when re-installing in JSON mode", async () => {
    ...
    expect(existsSync(join(upgradeMarket, "plugins", "close-rfc", "plugin.toml"))).toBe(true);
    ...
    expect(existsSync(join(upgradeMarket, "plugins", "close-rfc", "plugin.toml"))).toBe(false);
    expect(existsSync(join(upgradeMarket, "plugins", "close-rfc", ".codex-plugin", "plugin.json"))).toBe(true);
