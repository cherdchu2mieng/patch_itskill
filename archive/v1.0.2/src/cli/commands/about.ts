...
export function registerAbout(program: Command, version: string) {
  program
    ...
    .action(async () => {
      ...
      console.log(`\n  it-skill v${version}`);
      console.log(`  Digitized from IT TEAM Standard — itinfosv\n`);
      ...
      if (notDetected.length > 0) {
        console.log(`    (${notDetected.length} more agents supported — run 'it-skill agents' to see all)`);
      }

      // Installed profile hint
      const home = homedir();
      const manifestPath = join(home, '.claude/skills/.it-skill.json');
      if (existsSync(manifestPath)) {
        ...
        console.log('\n  Not initialized. Run: it-skill init');
      }
