...
async function getShortcutsPath(): Promise<string> {
  ...
  return join(homedir(), '.it-skill-shortcuts.json');
}
    ...
    .action(async (action?: string, name?: string, command?: string[]) => {
      ...
      if (!action || action === 'list' || action === 'ls') {
        if (shortcuts.length === 0) {
          ...
          console.log('    it-skill shortcut create <name> <command...>\n');
          return;
        }
        ...
        console.log(`\n  Delete: it-skill shortcut delete <number>\n`);
        return;
      }
      ...
      if (action === 'create' || action === 'add' || action === 'new') {
        ...
        if (!shortcutCmd) {
          const input = await p.text({
            ...
            placeholder: 'e.g., it-skill install -g -y',
          });
          if (p.isCancel(input)) return;
      ...
      console.log('  Usage:');
      console.log('    it-skill shortcut                        # list');
      console.log('    it-skill shortcut create <name> <cmd>    # create');
      console.log('    it-skill shortcut delete <number>        # delete');
      console.log('    it-skill shortcut <name>                 # run\n');
    });
}
