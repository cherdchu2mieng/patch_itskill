...
describe('CLI', () => {
  ...
  it('should show help', async () => {
    ...
    expect(result).toContain('it-skill');
...
describe('installer', () => {
  it('should discover bundled skills', async () => {
    ...
    expect(skills.some(s => s.name === 'close-rfc')).toBe(true);
    expect(skills.some(s => s.name === 'build-patch')).toBe(true);
    expect(skills.some(s => s.name === 'build-rfc')).toBe(true);
  });
