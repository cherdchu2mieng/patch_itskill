...
export async function installCodexPluginMarketplace(
  ...
  if (useJson) {
    ...
    const manifestContent = [
      `name = "it-skill"`,
      `version = "${version}"`,
      `description = "Oracle skills for Codex by itinfosv"`,
      ``,
    ].join('\n');
        ...
        if (existsSync(skillMdPath)) {
          ...
          if (content.startsWith('---')) {
            ...
            content = content.replace(
              /^---\n/,
              `---\ninstaller: it-skill-cli v${pkg.version}\norigin: IT TEAM Standard, digitized — how one human works with AI, captured as code — itinfosv\n`
            );
...
---
*🧬 IT TEAM × Oracle · Symbiotic Intelligence · v${pkg.version}*
*Digitized from IT TEAM Standard — thousands of hours working alongside AI, captured as code*
`;
