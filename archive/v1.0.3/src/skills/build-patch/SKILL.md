...
17. **Hotfix != Bypass (v2.9)**: A hotfix is fundamentally just a Change Request (CR) with a high priority. **It MUST still traverse every single gate.** Urgency does not justify suspending the Robust Patching protocol. Empirical human verification (Phase 4) is **non-negotiable** before proceeding to production delivery. See *references/Ref-002_hotfix_phase_jump_violation.md* for the case study.
...
### Phase 7: Production Delivery (v2.9)
**Goal**: Officially deliver the verified code to the main baseline and synchronize the feature branch.

1.  **Final Consent Gate**: **ASK for Human Permission** to perform the final delivery: "Do you want to proceed with Production Delivery? [y/N]". Default is "N". Wait (PAUSE) until "y" is received.
2.  **Authoritative Delivery**:
    *   **2.1 Merge Patch Workspace**: Merge or copy verified code from the `feature` branch to the main branch (`master` or `main`) in the **Patch Workspace Repo**. The `feature` branch MUST be pushed to GitHub for remote synchronization.
    *   **2.2 Push Target Repo**: Perform the final push of verified code to the **Target Repo** (Production/Main branch). 
3.  **Official Release Commit**: Perform the final commit on the main branch of the Patch Workspace with message: `release: vX.Y.Z [Project Name] - Ironclad Delivery 🛡️🌊`. Push to remote if applicable.
