// @it-skill-patch: itgo_complete@v1.6.2
import type { Command } from 'commander';
import * as p from '@clack/prompts';
import { agents, detectInstalledAgents, getDefaultAgents } from '../agents.js';
  ...
  program
    ...
    .action(async (actionOrProfile, skillName, options) => {
      ...
      for (const bin of bins) {
        ...
        console.log('  ' + bin.label.padEnd(20) + ' ' + status.padEnd(15) + ' ' + (v || '—').padEnd(10) + ' ' + displayPath);
      }
      
      const itSkillBin = join(homedir(), '.it-skill', 'bin');
      const inPath = process.env.PATH?.includes(itSkillBin);
      if (!inPath) {
          console.log('  ' + 'Bootstrap'.padEnd(20) + ' ' + '⚠️ Misaligned'.padEnd(15) + ' ' + '—'.padEnd(10) + ' (Run \'it-skill itgo bootstrap\' to fix)');
      } else {
          console.log('  ' + 'Bootstrap'.padEnd(20) + ' ' + '✅ Aligned'.padEnd(15) + ' ' + '—'.padEnd(10) + ' (' + itSkillBin + ')');
      }
      console.log('  ' + '-'.repeat(85));
      console.log('');

      // ── Usage Stats Mining (CR-004 - Session-based Adoption) ──────────────
              ...
              for (const f of files) {
                  try {
                      ...
                      for (const cmd of itCommands) {
                          // If command appears in this session log, count it as one "Adopted Session"
                          if (logContent.includes('/' + cmd)) {
                              usageStats[cmd] = (usageStats[cmd] || 0) + 1;
                          }
                      }
                  } catch {}
              }
      ...
      for (const agentName of targetAgents) {
        ...
        if (skillData.length === 0) {
          ...
          console.log('  ' + '-'.repeat(85));
          console.log('  ' + '#'.padEnd(4) + 'Skill'.padEnd(32) + 'Manager'.padEnd(12) + 'Version'.padEnd(12) + 'Usage'.padEnd(12) + 'Status');
          console.log('  ' + '-'.repeat(85));
          ...
          for (const s of skillData) {
            ...
            // Clean table: Only numbers, fixed padding
            const usageStr = usage > 0 ? usage.toString() : '—';
            console.log('  ' + String(i).padEnd(4) + s.name.padEnd(32) + s.manager.padEnd(12) + s.version.padEnd(12) + usageStr.padEnd(12) + s.status);
            i++;
          }
          console.log('  ' + '-'.repeat(85));
