// @it-skill-patch: fix_already_installed_check@v1.1.0 remove_install_default_logic@v1.1.0
...
export function registerInstall(program: Command, version: string) {
  program
    ...
    .option('-p, --profile <name>', 'Install a skill profile (minimal, standard, full, lab)', 'standard')
    ...
    .action(async (options, cmd) => {
      p.intro(`🛡️ IT-Skill Installer v${version}`);
...
const alreadyInstalled = detected.filter((a) => {
        ...
        p.outro('✅ IT-Skill suite installed!');

        // Awakening — show CLI commands on first install
        console.log(`
  🛡️ IT-Skill v${version} — Awakened

  CLI Commands:
    it-skill agents             # list supported agents
    it-skill about              # prereqs + system status
    it-skill list -g            # show installed skills
    it-skill select -g          # interactive skill picker
    it-skill install -g -y      # reinstall all skills
    it-skill uninstall -g -y    # remove all skills
