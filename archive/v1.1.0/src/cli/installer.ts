// @it-skill-patch: rich_manifest@v1.1.0
...
/** Return the it-skill marketplace bundle directory path. */
export function getCodexMarketplaceDir(homeDir?: string): string {
  const home = homeDir ?? homedir();
  return join(home, '.codex', '.tmp', 'bundled-marketplaces', 'it-skill');
}

/**
 * Return the Codex 0.130+ plugin cache directory for it-skill.
...
export function getCodexPluginCacheDir(homeDir?: string): string {
  const home = homeDir ?? homedir();
  return join(home, '.codex', 'plugins', 'cache', 'it-skill');
}
 ...
 *   [marketplaces.it-skill]  source_type + source
 *   [plugins."<skill>@it-skill"]  enabled = true
 *
...
export async function installCodexPluginMarketplace(
  ...
  if (useJson) {
    ...
    const marketplaceManifest = {
      name: 'it-skill',
      interface: {
        displayName: 'IT-Skill CLI Dashboard',
      },
      plugins: skills
        .filter((s) => !s.hidden)
        .map((s) => ({
  ...
  const marketplaceSection = `[marketplaces.it-skill]`;
  ...
  for (const skill of skills) {
    ...
    const pluginKey = `[plugins."${skill.name}@it-skill"]`;
...
// Check if an installed skill was installed by it-skill-cli
...
export async function installSkills(
  ...
  // remove it-skill-managed skills that are no longer in the target profile.
  ...
  if (options.profileExplicit && profileSkillNames !== null) {
    ...
    for (const agentName of targetAgents) {
      ...
      console.log(`\n⚠  Profile alignment — will REMOVE it-skill-managed skills not in '${options.profile}': ${toRemove.join(', ')}\n`);
  ...
  for (const agentName of targetAgents) {
    ...
    // Only removes skills that: 1) have installer: it-skill-cli marker, 2) no longer exist in source
    ...
    // Build rich skills manifest with specific versions
    const skillListWithVersions = [];
    for (const skill of agentSkillsToInstall) {
      // In it-skill-cli, we stamp all installed skills with the CLI version
      skillListWithVersions.push({ name: skill.name, version: 'v' + pkg.version });
    }
...
Installed by: **it-skill-cli v${pkg.version}**
...
\`\`\`bash
bunx --bun it-skill@github:Soul-Brews-Studio/it-skill-cli#v${pkg.version} install -y -g
\`\`\`
`;
...
`;
    ...
    if (agentName === 'opencode') {
      ...
      const hookSrc = join(dirname(import.meta.path), '..', 'hooks', 'opencode', 'it-skill.ts');
      if (existsSync(hookSrc)) {
        await cp(hookSrc, join(pluginDir, 'it-skill.ts'), shellMode);
        p.log.success(`OpenCode plugin: ${pluginDir}/it-skill.ts`);
      }
    }
    ...
    //   `install --profile <name>` (explicit) → ALIGN: remove it-skill-managed skills not in target
