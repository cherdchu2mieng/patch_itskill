    ...
    .action(async (actionOrProfile, skillName, options) => {
      ...
      if (actionOrProfile && profiles[actionOrProfile as keyof typeof profiles]) {
        ...
        if (targetAgents.length === 0) {
            // CR-005: Robust auto-detection same as 'install' command
      ...
      if (actionOrProfile && !profiles[actionOrProfile as keyof typeof profiles] && actionOrProfile !== 'refresh') {
      ...
      // ── Fleet Environment Block (CR-003/004 + CR-005 Bootstrap) ──────────
      ...
      // CR-005: Bootstrap Alignment Check
      const itSkillDir = join(homedir(), '.it-skill', 'bin');
      const inPath = process.env.PATH?.includes(itSkillDir);
      const bootstrapStatus = inPath ? '✅ Aligned' : '⚠️ Misaligned (PATH)';
      console.log(`  ${'Bootstrap'.padEnd(20)} ${bootstrapStatus.padEnd(15)} — (${itSkillDir})`);
